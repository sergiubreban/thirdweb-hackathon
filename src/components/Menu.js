...
const Menu = ({ hasClaimedNFT, openTour }) => {
  ...
  return <Box h='10vh' w='100vw' { ...(hasClaimedNFT && { bg: '#1A202C' }) } transition='background 1s linear' >
    <Flex justify='center' h='100%' alignItems='flex-end' w='100%'>
      ...
      <Stack spacing={ 2 } position='absolute' direction='row-reverse' alignItems='center' top='3vh' right='0' mx='4' className='menu-actions'>
        ...
        { hasClaimedNFT && <Text mx='2' onClick={ () => openTour && openTour() } cursor='pointer' color={ textColor }>Help</Text> }
      </Stack>
    </Flex>
  </Box>
}
