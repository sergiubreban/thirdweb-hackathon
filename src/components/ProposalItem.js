  ...
  return (
    ...
    >
      <Stack direction={'row'} spacing="1">
        <Tooltip label="Genre">
          <Box bg={`proposalStatus.genre`} px="1" borderRadius="5px">
            <Text color="#000">{genre}</Text>
          </Box>
        </Tooltip>
        <Tooltip label="Status">
          <Box bg={`proposalStatus.${proposal.state}`} px="1" borderRadius="5px">
            {ProposalStateMapper[proposal.state]}
          </Box>
        </Tooltip>
        {hasVoted && (
          <Box bg={`proposalStatus.1`} px="1" borderRadius="5px">
            voted
          </Box>
        )}
      </Stack>
      <Flex justify="space-between">
        <Text>
          Proposer: <Address address={proposal.proposer} />
        </Text>
      ...
      <Flex justify="space-around" className="proposal-vote">
        {hasVoted ? (
          ...
          proposal.votes.map((vote) => (
            <Button variant="simple" key={vote.type} isLoading={isVoting} onClick={() => voteProposal(vote.type)}>
              {vote.label}
            </Button>
          ))
        )}
    ...
    <Box>
      ...
      {showMore &&
        (youtubeId ? (
          ...
          <>
            <Button _hover={{ color: '#fff' }} variant="outline" onClick={() => setShow(true)}>
              Go to song
            </Button>
            {show && (
              <iframe
